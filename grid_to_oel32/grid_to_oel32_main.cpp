...
grid_to_oel32::config_t make_config(char* argv[]) noexcept {
    grid_to_oel32::config_t cfg;
    try {
        std::string argv_s[6];
...
int main(int argc, char* argv[]) {
    if (argc < 6) {
        printf("usage> grid_to_oel32 <GRID_NAME> <INPUT_PATH> <OUTPUT_PATH> <OUTPUT_BUF_SIZE:MB> <IS_DENSE_FORMAT>");
        return -1;
    }

    grid_to_oel32::config_t const cfg = make_config(argv);
    ...
    grid_to_oel32* prog = new grid_to_oel32(cfg);
    if (prog->run()) {
        printf("total edge : %lu\n", prog->get_total_edge());
    }
    else {
        printf("grid_to_oel32 returns error!\n");
    }
    delete prog;
